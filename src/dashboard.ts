...
const GROUPS: Group[] = [
  {
    ...
    eps: [
      { method: "GET", path: "/api/jin/latest", free: true, desc: "最新観測日の指数（excl_promo / incl_promo・matched・base_date・coverage・passthrough_gap）。" },
      { method: "GET", path: "/api/jin/series", price: "$0.01", desc: "指数の時系列（from / to は任意）。" },
      { method: "GET", path: "/api/jin/movers", price: "$0.02", desc: "その日動いた品目（特売タグ付き。date は任意）。" },
    ],
  },
  {
    ...
    eps: [
      ...
      { method: "GET", path: "/api/alpha/portfolio/current", price: "$0.01", desc: "米ポートフォリオ 現在10銘柄（ticker / weight / thesis）。" },
      { method: "GET", path: "/api/alpha/portfolio/scorecard", price: "$0.01", desc: "米 catalyst hit-rate ＋ SPY / QQQ 累積リターン。" },
      { method: "GET", path: "/api/alpha/jp/portfolio/current", price: "$0.01", desc: "日本ポートフォリオ 現在10銘柄。" },
      { method: "GET", path: "/api/alpha/jp/scorecard", price: "$0.01", desc: "日本 hit-rate（ベンチ指数なし）。" },
      { method: "GET", path: "/api/alpha/jp/catalysts", price: "$0.01", desc: "日本 dated catalysts 一覧。" },
